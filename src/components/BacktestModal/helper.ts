...
export function stringifyRequestData(
  ...
  try {
    ...
    }__${data.maxVolMultiplier ? data.maxVolMultiplier : '0'}__${data.reverseCopy ? '1' : '0'}${
...
export function parseRequestData(params: string | undefined, protocol: ProtocolEnum) {
  ...
  if (listData.length < 12) return result
  ...
  try {
    ...
    if (parseData.some((data) => !data || isNaN(data as number)) || !tokenAddresses?.length) return result
