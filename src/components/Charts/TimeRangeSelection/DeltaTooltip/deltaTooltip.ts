...
import { MultiTouchCrosshairPaneView, TooltipCrosshairLineData } from './crosshair'
import { MultiTouchChartEvents, MultiTouchInteraction } from './events'
import { DeltaSingleTooltipData, DeltaTooltipData, DeltaTooltipPaneView } from './renderer'
...
export class DeltaTooltipPrimitive implements ISeriesPrimitive<Time> {
  ...
  enable(enabled: boolean) {
    if (!this._touchChartEvents) return
    this._touchChartEvents.disabled = !enabled
  }
  ...
  private _seriesLineColor(): string {
    ...
    return (seriesOptions as LineStyleOptions).color || (seriesOptions as AreaStyleOptions).lineColor || '#888'
  }
  ...
  private _showTooltip(interactions: MultiTouchInteraction) {
    ...
    for (let i = 0; i < Math.min(2, interactions.points.length); i++) {
      ...
      if (data) {
        ...
        timeValues.push([data.time, point.index])
    ...
    if (priceValues.length > 1) {
      ...
      const firstTime = correctOrder ? timeValues[0][0] : timeValues[1][0]
      const secondPrice = correctOrder ? priceValues[1][0] : priceValues[0][0]
      const secondTime = correctOrder ? timeValues[1][0] : timeValues[0][0]
      const priceChange = secondPrice - firstPrice
      const pctChange = (100 * priceChange) / firstPrice
      const positive = priceChange >= 0
      const days = Math.ceil((Number(secondTime) - Number(firstTime)) / 3600 / 24)
      deltaContent.deltaTopLine = `${days} days`
