...
function determineChartX(chartElement: HTMLDivElement, chart: IChartApi, mouseX: number): number | null {
...
export class MultiTouchChartEvents {
  ...
  _addKeyEventListener(
    target: HTMLDivElement,
    eventType: 'keyup' | 'keydown',
    handler: (event: KeyboardEvent) => void
  ): void {
    const keyHandler = handler.bind(this)
    target.addEventListener(eventType, keyHandler)
    const unSubscriber = () => {
      target.removeEventListener(eventType, keyHandler)
    }
