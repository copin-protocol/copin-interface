...
const styles = {
  ...
  tooltipLineColors: [themeColors.neutral7, themeColors.neutral6, themeColors.neutral5],

  deltaFontWeights: [590, 400] as number[],
  deltaFontSizes: [13, 12] as number[],
...
type CalculatedDrawingPositions = CalculatedVerticalDrawingPositions & CalculatedHorizontalDrawingPositions

function calculateVerticalDrawingPositions(data: DeltaTooltipData): CalculatedVerticalDrawingPositions {
  const mainY = data.topSpacing

  const leftTooltipHeight =
    data.tooltips.length < 1 ? 0 : determineSectionHeight(data.tooltips[0].lineContent, styles.tooltipLineLineHeights)
  const rightTooltipHeight =
    data.tooltips.length < 2 ? 0 : determineSectionHeight(data.tooltips[1].lineContent, styles.tooltipLineLineHeights)
...
class DeltaTooltipPaneRenderer implements ISeriesPrimitivePaneRenderer {
  ...
  _drawTooltipsText(ctx: CanvasRenderingContext2D, positions: CalculatedDrawingPositions) {
    ...
    this._data.tooltips.forEach((tooltip: DeltaSingleTooltipData, tooltipIndex: number) => {
      ...
      let y = positions.mainY + (tooltipIndex === 0 ? positions.leftTooltipTextY : positions.rightTooltipTextY)
