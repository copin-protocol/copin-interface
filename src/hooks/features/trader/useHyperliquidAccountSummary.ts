...
const useHyperliquidAccountSummary = ({
  ...
  return useMemo(() => {
    ...
    hlAccountData?.assetPositions,
    hlAccountData?.marginSummary?.accountValue,
    hlAccountData?.marginSummary?.totalMarginUsed,
    hlAccountData?.marginSummary?.totalNtlPos,
    hlAccountData?.withdrawable,
    hlAccountSpotData,
  ])
}
