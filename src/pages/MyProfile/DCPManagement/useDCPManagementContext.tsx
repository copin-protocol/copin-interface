...
export type DCPManagementContextValues = {
...
const DCPManagementContext = createContext<DCPManagementContextValues>({} as DCPManagementContextValues)
...
export function DCPManagementProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const foundWallet = dcpWallets.find((data) => (walletStored ? data.id === walletStored.id : false))
    if (!!activeWallet) {
      if (foundWallet) return
      setActiveWallet(dcpWallets[0])
    } else {
  ...
  useEffect(() => {
    ...
    setActiveWallet((prev) => {
      const foundWallet = dcpWallets.find((data) => data.id === defaultWalletId)
      if (foundWallet) return foundWallet
      return prev
    })
    setSearchParams({ [URL_PARAM_KEYS.MY_MANAGEMENT_WALLET_ID]: null })
  }, [loadingCopyWallets, locationState, dcpWallets])
  useEffect(() => {
    if (!activeWallet) return
    sessionStorage.setItem(STORAGE_KEYS.USER_COPY_WALLET_DCP, JSON.stringify(activeWallet) ?? '')
  }, [activeWallet])
  ...
  const {
    ...
    storageKey: STORAGE_KEYS.USER_COPY_TRADERS_DCP,
  })
  ...
  const contextValues = useMemo(() => {
    const result: DCPManagementContextValues = {
      dcpWallets,
      allCopyTrades,
      copyTrades,
      isLoadingCopyTrades,
  ...
  return <DCPManagementContext.Provider value={contextValues}>{children}</DCPManagementContext.Provider>
}
const useDCPManagementContext = () => {
  const values = useContext(DCPManagementContext)
  if (!Object.keys(values).length) throw Error('useDCPManagementContext need to be used in DCPManagementProvider')
  return values
}
export default useDCPManagementContext
