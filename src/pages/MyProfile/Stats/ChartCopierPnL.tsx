...
export default function ChartCopierPnL({
  data,
  isLoading,
  from,
  to,
  ...
  const chartData: LineData[] = useMemo(() => {
    if (!data || !generateData) return []
    return generateData.map((e) => {
      return {
        value: e.amount,
  ...
  useEffect(() => {
    ...
    const container = document.getElementById('chart-pnl-container')
    const chart = createChart(container ? container : 'chart-pnl-container', {
      height: 160,
      rightPriceScale: {
        autoScale: true,
        visible: false,
    ...
    <>
      ...
      {data && chartData && (
        <Box px={3}>
          ...
          <Box mt={1} sx={{ position: 'relative' }} minHeight={90}>
            <div id="chart-pnl-container" />
          </Box>
        </Box>
      )}
    </>
