...
const VaultOnchainPositionContainer = ({ smartWalletAddress }: { smartWalletAddress: string }) => {
  const {
    onchainPositions,
    positionsMapping,
...
const VaultOnchainPositions = memo(function VaultOnchainPositionsRenderer({
  smartWalletAddress,
  ...
  const { isValid, alert } = useRequiredChain({
    chainId: exchange === CopyTradePlatformEnum.GNS_V8 ? ARBITRUM_CHAIN : OPTIMISM_CHAIN,
  })
  return isValid ? (
    !!smartWalletAddress ? (
      <VaultOnchainPositionContainer smartWalletAddress={smartWalletAddress} />
    ) : (
      <></>
    )
  ) : (
...
export default VaultOnchainPositions
