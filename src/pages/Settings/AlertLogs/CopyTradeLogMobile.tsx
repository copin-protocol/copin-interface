...
export default function CopyTradeLogMobile({
  data,
  isLoading,
  externalSource,
  noDataComponent,
  ...
  return (
    <Box sx={{ p: 3, bg: 'neutral6' }}>
      <Box sx={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: 3 }}>
        ...
        <Property label={alertLogTitles.copyTitle} value={renderProps.copyTitle?.(data)} />
      </Box>
      ...
      <Divider color="neutral5" my={2} />
      <Box sx={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: 3 }}>
        <Property label={alertLogTitles.targetWallet} value={renderProps.targetWallet?.(data)} />
        <Property
          label={alertLogTitles.targetAction}
