...
import WatchlistLogMobile from './WatchlistLogMobile'
import { ExternalSource } from './configs'

export default function WatchlistLogs({
  data,
  isLoading = false,
  currentSort,
  changeCurrentSort,
  ...
  return (
    <Flex flex={1} flexDirection="column" sx={{ overflow: 'hidden' }}>
      {lg ? (
        ...
        <WatchlistLogMobile data={data?.data} isLoading={isLoading} externalSource={externalSource} />
      )}
    </Flex>
  )
}
